...
&НаСервере
Функция НайтиИлиСоздатьМатериал(Артикул, Наим) Экспорт

	Спр = Справочники.НоменклатураМатериалов;

	Арт  = СокрЛП(Строка(Артикул));
	Имя  = СокрЛП(Строка(Наим));

	МатСсылка = Неопределено;

	// 1) Поиск по артикулу (если реквизит есть)
	Если Арт <> "" Тогда
		Попытка
			МатСсылка = Спр.НайтиПоРеквизиту("Артикул", Арт);
		Исключение
		КонецПопытки;
	...
	Если МатСсылка = Неопределено И Имя <> "" Тогда
		МатСсылка = Спр.НайтиПоНаименованию(Имя);
		Если НЕ ЗначениеЗаполнено(МатСсылка) Тогда
			МатСсылка = Неопределено;
		КонецЕсли;
	КонецЕсли;
	...
	// 3) Создание
	Если МатСсылка = Неопределено Тогда

		Мат = Спр.СоздатьЭлемент();

		Если Имя <> "" Тогда
			Мат.Наименование = Имя;
		ИначеЕсли Арт <> "" Тогда
			Мат.Наименование = Арт;
		Иначе
			Мат.Наименование = "Материал";
		КонецЕсли;
